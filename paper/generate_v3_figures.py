"""Generate v3 publication figures (enlarged internal fonts).

Derived from generate_v2_figures.py: identical data, palette, and layout;
only the internal font sizes are scaled up for readability (the v3 manuscript
reads larger panels). Outputs go to figures_v3/ unless FIG_DIR is set.
...
FIG_DIR = PAPER_DIR / os.environ.get("FIG_DIR", "figures_v3")
...
FONT_SCALE = 1.10  # v3: ~10% larger internal fonts than v2 (18% breaks fig1 layout)
    ...
    plt.rcParams.update(
        {
            ...
            "font.size": 7.4 * s,
            "axes.labelsize": 7.4 * s,
            "axes.titlesize": 8.0 * s,
            "axes.titleweight": "semibold",
            "axes.linewidth": 0.72,
            "axes.spines.top": False,
            "axes.spines.right": False,
            "axes.axisbelow": True,
            "xtick.labelsize": 6.6 * s,
            "ytick.labelsize": 6.6 * s,
            "xtick.major.width": 0.65,
            "ytick.major.width": 0.65,
            "xtick.major.size": 2.8,
            "ytick.major.size": 2.8,
            "legend.fontsize": 6.4 * s,
...
def figure1_workflow() -> plt.Figure:
    ...
    ax.text(wt[0], wt[1] + 0.070, "WT", ha="center", fontsize=7.5, weight="bold")
    ax.text(0.50, 0.92, "Active", ha="center", va="bottom", fontsize=7.3)
    ax.text(0.03, 0.04, "E1", ha="left", va="top", fontsize=7.3)
    ax.text(0.97, 0.04, "E2", ha="right", va="top", fontsize=7.3)
    ...
    for center, label, conflict in ((0.27, "supported", False), (0.76, "conflict", True)):
        ...
        ax.text(center, 0.19, label, ha="center", fontsize=7.2, color=INK)
    ax.text(0.52, 0.89, r"near in $c$  $\nRightarrow$  near in $d$", ha="center", fontsize=7.1)
    ax.axis("off")
    ...
    ax.text(0.48, -0.18, "shared", ha="center", fontsize=7.1, color=PALETTE["blue_main"])
    ax.text(0.08, 0.48, "fine", va="center", fontsize=7.1, color=PALETTE["red_strong"], rotation=90)
    ax.text(0, -0.58, r"$u_1=A-(E1+E2)$", ha="center", fontsize=7.5)
    ax.text(0, -0.82, r"$u_2=E1-E2$", ha="center", fontsize=7.5)
    ax.set_xticks([])
    ax.set_yticks([])
    ...
    for y, label in rows:
        ax.text(0.03, y, label, va="center", fontsize=6.9)
        if label == "LOO":
            ...
            ax.text(0.72, y, "select", ha="center", va="center", fontsize=6.6)
    ax.axis("off")
    return fig
...
def figure2_support(data: dict[str, object]) -> plt.Figure:
    ...
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            value = matrix[i, j]
            ax.text(j, i, f"{value:.2f}", ha="center", va="center", fontsize=6.6, color="white" if value > 0.36 else INK)
    ...
    colorbar.set_label("MAE", fontsize=7.1, labelpad=3)
...
def figure3_resolution(data: dict[str, object]) -> plt.Figure:
    ...
    for name, point in zip(names, true_points):
        ...
        ax.annotate(
            label, point, xytext=label_xy, textcoords="data", fontsize=6.4, color=INK,
            ha="left", va="center",
            bbox={"facecolor": "white", "edgecolor": "none", "pad": 0.35, "alpha": 0.94},
    ...
    ax.legend(loc="lower right", bbox_to_anchor=(0.995, 0.015), ncol=3, columnspacing=0.65,
              handletextpad=0.25, borderaxespad=0.0, fontsize=6.4)
    ...
    for yi, value in zip(y, values):
        ax.text(value + 0.025, yi, f"{value:.2f}", va="center", fontsize=6.6)
    ...
    ax.set_ylabel("MAE")
    ax.set_ylim(0, 0.78)
    ax.legend(loc="upper left", handlelength=1.2, handletextpad=0.3, fontsize=6.6)
    return fig
...
def figure4_synthetic(data: dict[str, object]) -> plt.Figure:
    ...
    for delta, color, marker, linestyle in delta_styles:
        ...
        ax.annotate(
            ...
            textcoords="data", ha="left", va="center", fontsize=6.2, color=color,
        )
    floor_row = [
        row for row in summary
    ...
    for resolution, field, label, color, marker, linestyle in series:
        ...
        ax.plot((0.0, 0.6, 1.2), ys, color=color, marker=marker, ls=linestyle,
                ...
                label=label)
    ax.legend(loc="lower right", fontsize=6.4, handlelength=1.4,
              handletextpad=0.35, borderaxespad=0.25, labelspacing=0.28)
...
def figure4_evidence(data: dict[str, object]) -> plt.Figure:
    ...
    for yi, value in zip(y, values):
        ax.text(value + 0.007, yi, f"{value:.3f}", va="center", fontsize=6.7)
    ...
    for label in ("Mean", "ESM-2 PCA", "LLR"):
        ...
        ax.annotate(
            ...
            ha="left", va="center", fontsize=6.7, color=color,
    ...
    for bars in (bars1, bars2):
        for bar in bars:
            ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.009, f"{bar.get_height():.2f}", ha="center", fontsize=6.6)
    ax.set_xticks(x)
    ax.set_xticklabels(labels)
    ax.set_ylabel("Group MAE")
    ax.set_xlabel("* identity columns constant\nin M290/L301 training", fontsize=6.7, labelpad=3)
    ax.set_ylim(0, 0.41)
...
def figure5_alignment(data: dict[str, object]) -> plt.Figure:
            ...
            for value, state in zip(values, ("active", "i1i2", "unclassified")):
                ...
                if value and fraction >= 0.065:
                    ...
                    ax.text(center + offset, bottom + fraction / 2, str(value), ha="center", va="center",
                            fontsize=6.7, color=text_color, weight="bold" if fraction > 0.22 else "normal")
                bottom += fraction
    ...
    for y, interval, color in zip((1, 0), ranges, colors):
        ...
        ax.text(interval[1] + 0.4, y, f"{interval[0]:.0f}-{interval[1]:.0f}",
                va="center", fontsize=6.9, color=color)
    ax.set_yticks([1, 0])
...
def write_manifest(outputs: list[Path]) -> None:
    ...
    manifest = {
        "generator": str(generator_path.relative_to(ROOT)).replace("\\", "/"),
